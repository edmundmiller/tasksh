...
func NewTestTasksh(t testing.TB) *TestTasksh {
...
func findTaskshBinary() string {
	...
	// Try the binary in the project root
	binaryPath := filepath.Join(projectRoot, "tasksh")
	if _, err := os.Stat(binaryPath); err == nil {
		return binaryPath
	}
	
	// Try building it if it doesn't exist
	cmd := exec.Command("go", "build", "-o", "tasksh", "./cmd/tasksh")
	cmd.Dir = projectRoot
	...
	panic("tasksh binary not found - run 'go build -o tasksh ./cmd/tasksh' first")
}
