...
func TestGetSimilarTasks(t *testing.T) {
	...
	tasks := []struct {
		...
		{
			&Task{UUID: "1", Description: "Write unit tests", Project: "tasksh", Priority: "H"},
			2.0, 3.0,
		},
		{
			&Task{UUID: "2", Description: "Write integration tests", Project: "tasksh", Priority: "M"},
			3.0, 4.5,
		},
		{
			&Task{UUID: "3", Description: "Fix bug in parser", Project: "other", Priority: "H"},
			1.0, 1.5,
		},
	}
	...
	for _, entry := range similar {
		if entry.Project != "tasksh" && !contains(entry.Description, "tests") {
...
func TestParseTimeInput(t *testing.T) {
	...
	for _, tt := range tests {
		result := parseTimeInput(tt.input)
		if result != tt.expected {
			t.Errorf("parseTimeInput(%q) = %f, want %f", tt.input, result, tt.expected)
		}
	}
}
